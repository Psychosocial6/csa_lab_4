...
Гарвардская архитектура: 2 раздельных памяти для данных и для инструкций. Используются 32-битные машинные слова. Присутствуют несколько видов адресации: прямая, косвенная, относительная, непосредственная загрузка операнда. Обе памяти являются байт-адресуемыми
- **Память данных:** размер 1024 байта. Хранит переменные, строки, стек.
- **Память команд:** размер 2^24 байт (адреса с `0x0` до `0xFFFFFF`), доступна только для чтения. Хранит инструкции.

Минимальная единица данных - машинное слово (так как команды для работы с отдельными байтами отсутствуют)
...
- **Литералы:**
  ...
  * Числа, занимающие 32 бита или инициализирующиеся в секции `.data` занимают 32 бита - 4 байта в памяти данных.
...
- Последовательное выполнение: после каждой инструкции ```PC``` увеличивается на 4 (если инструкция не изменяет ```PC``` явно).
- Переходы: ```JMP```, ```JZ```, ```JN```, ```JC```, ```JNC``` изменяют ```PC``` на целевой адрес или на ```PC + 4 + смещение```.
...
- **SC:** счетчик тактов исполняемой инструкции

**Описание управляющих сигналов:**
- **latch_IR, latch_PC, latch_SC:** управляюшие синалы, защелкиваюшие значения в соответствующих регистрах
...
- **sel_SP:** управляющий сигнал мультиплексора перед `SP`, осуществляет выбор нового значения `SP`: `SP+4` или `SP-4`
...
2. ```cat_overwrite.asm``` - чтение ввода из порта ```0xFFFFF0``` с малыми промежутками (невозможность прочитать все вводимые символы)
3. ```hello.asm``` - вывод текста в порт ```0xFFFFF1```
4. ```hello_user_name.asm``` - запрос имени пользователя, вывод приветствия с соответствующим именем
5. ```sort.asm``` - сортировка массива пузырьком
6. ```double_arith.asm``` - работа числами в 64 бита
7. ```euler6.asm``` - разность квадрата суммы и суммы квадратов
